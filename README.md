...
- [ ] **Phase 0** — Scaffold, gitignore discipline, build-in-public setup
- [ ] **Phase 1** — Ingestion layer → bronze Parquet
- [ ] **Phase 2** — DuckDB warehouse + silver daily grain (two dbt targets wired)
- [ ] **Phase 3** — dbt marts + NHANES/ACSM benchmarks + tests
...
make ingest      # Phase 1+
make build       # dbt build against the dev (DuckDB) target
